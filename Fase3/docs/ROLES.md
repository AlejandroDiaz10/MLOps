# Matriz de Roles y Responsabilidades – Fase 3 (Proyecto MLOps)
...
|------|-------------------------------|-------------------------------|
| **Data Engineer (DE)** | - Recolecta y limpia los datos.<br>- Estandariza formatos y valida esquemas.<br>- Genera los conjuntos intermedios (`data/interim`). | `fase3/dataset.py`<br>`data/interim/` |
| **Data Scientist (DS)** | - Realiza *feature engineering* y partición *train/test*.<br>- Desarrolla y entrena modelos (`RandomForest`, `LogisticRegression`, `DecisionTree`).<br>- Evalúa métricas (Accuracy, Precision, Recall, F1, AUC).<br>- Genera visualizaciones de desempeño. | `fase3/features.py`<br>`fase3/modeling/train.py`<br>`fase3/modeling/predict.py`<br>`reports/figures/` |
